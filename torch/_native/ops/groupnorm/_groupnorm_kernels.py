...
from torch._vendor.quack.copy_utils import copy, predicate_k
from torch._vendor.quack.cute_dsl_utils import torch2cute_dtype_map
from torch._vendor.quack.layout_utils import expand
from torch._vendor.quack.reduce import row_reduce
from torch._vendor.quack.reduction_base import ReductionBase
from torch._vendor.quack.utils import fill_oob

import torch
from torch import Tensor


_TORCH2CUTE_DTYPE = torch2cute_dtype_map
...
def _load_affine_param(
    ...
    for rest_v in cutlass.range_constexpr(tXrP.shape[0][1]):
        for rest_k in cutlass.range_constexpr(tXrP.shape[2]):
            base_col = tXcX[rest_v, 0, rest_k][1]
...
class GroupNormBackward(ReductionBase):
    ...
    def __init__(self, dtype: type[cutlass.Numeric], K: int, cpg: int, HxW: int):
        super().__init__(dtype, N=K, stage=4, reduction_dtype=Float32)
        self.K = K
        self.cpg = cpg
        self.HxW = HxW
    ...
    def kernel(
        ...
        if const_expr(mbar_ptr is not None):
            mbar_full_ptr, mbar_empty_ptr = mbar_ptr, mbar_ptr + 4
        else:
        ...
        for bidx in cutlass.range(bidx_start, cute.ceil_div(M, tiler_mn[0]), gdim):
            ...
            reduce_stage = stage * 2
            mean_wdy = (
                row_reduce(
                    wdy,
                    cute.ReductionOp.ADD,
                    threads_per_row,
                    reduction_buffer[None, None, reduce_stage],
                    (
                        mbar_full_ptr + reduce_stage
                        if const_expr(self.cluster_n > 1)
                        else None
                    ),
            ...
            mean_xhat_wdy = (
                row_reduce(
                    ...
                    cute.ReductionOp.ADD,
                    threads_per_row,
                    reduction_buffer[None, None, reduce_stage + 1],
                    (
                        mbar_full_ptr + reduce_stage + 1
                        if const_expr(self.cluster_n > 1)
                        else None
                    ),
                    phase=consumer_phase,
