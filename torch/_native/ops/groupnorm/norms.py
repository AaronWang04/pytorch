...
def _reshape_2d(t: torch.Tensor, M: int, K: int) -> torch.Tensor:
    if t.ndim == 2 and t.shape[0] == M and t.shape[1] == K and t.is_contiguous():
        return t
    return t.reshape(M, K).contiguous()


def _reshape_1d(t: torch.Tensor, size: int) -> torch.Tensor:
    if t.ndim == 1 and t.shape[0] == size and t.is_contiguous():
        return t
...
def cutedsl_groupnorm_fwd(
    ...
    x = _reshape_2d(input, M, K)

    out = torch.empty(M, K, device=x.device, dtype=x.dtype)
    mean = torch.empty(M, device=x.device, dtype=torch.float32)
    rstd = torch.empty(M, device=x.device, dtype=torch.float32)
    if weight is not None:
        weight = _reshape_1d(weight, C)
    if bias is not None:
        bias = _reshape_1d(bias, C)
...
def cutedsl_groupnorm_bwd(
    ...
    x = _reshape_2d(input, M, K)
    dout = _reshape_2d(grad_out, M, K)
    mean_flat = _reshape_1d(mean, M)
    rstd_flat = _reshape_1d(rstd, M)
    if weight is not None:
        weight = _reshape_1d(weight, C)
    ...
    dw = _empty_zeroed(C, x.device) if output_mask[1] and weight is not None else None
    db = _empty_zeroed(C, x.device) if output_mask[2] else None
